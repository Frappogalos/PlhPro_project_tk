        ...
        if self.moving:
            for car in Car.cars_list:
                if self != car and self.direction == car.direction and self.lane == car.lane:
                    if (self.direction == 1 or self.direction == 3) and abs(self.x - car.x) < 60:
                        self.moving = False
                        self.stopped = car
                    elif (self.direction == 2 or self.direction == 4) and abs(self.y - car.y) < 60:
                        self.moving = False
                        self.stopped = car
        if self.moving and -100 < self.x < 1700 and -100 < self.y < 1000:
            ...
            self.delete_car()
        if not self.moving:
            if (self.direction == 1 or self.direction == 3) and abs(self.x - self.stopped.x) > 80:
                self.moving = True
                self.stopped = None
                self.root.after(500, self.move_car)
            elif (self.direction == 2 or self.direction == 4) and abs(self.y - self.stopped.y) > 80:
                self.moving = True
                self.stopped = None
                self.root.after(500, self.move_car)
