        ...
        if self.moving:
            for car in Car.cars_list:
                if self != car and self.direction == car.direction and self.lane == car.lane:
                    if (self.direction == 1 or self.direction == 3) and abs(self.x - car.x) < 120:
                        self.moving = False
                        self.speed = (0, 0)
                        self.stopped = car
                        self.root.after(30, self.move_car)
                    elif (self.direction == 2 or self.direction == 4) and abs(self.y - car.y) < 120:
                        self.moving = False
                        self.speed = (0, 0)
                        self.stopped = car
                        self.root.after(30, self.move_car)
        if self.direction == 1 and 1000 < self.x < 1100:
            self.speed = 0, 0
        if self.moving:
